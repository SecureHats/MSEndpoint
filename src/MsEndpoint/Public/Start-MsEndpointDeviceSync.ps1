function Start-MsEndpointDeviceSync {
    <#
    .Synopsis
...
#>
    ...
    process {
        ...
        if ([string]::IsNullOrEmpty($deviceObjectId)) {
            Write-Error "No valid devicename of deviceId has been specified"
        }

        $syncUri = '{0}/{1}/syncDevice' -f $baseUri, $deviceObjectId
        try {
            Write-Verbose "Starting Sync for endpoint [$($syncUri)]"
