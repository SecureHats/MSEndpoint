function Start-MsEndpointDeviceClean {
    <#
    .Synopsis
...
#>
    ...
    process {
        ...
        $syncUri = '{0}/{1}/cleanWindowsDevice' -f $baseUri, $deviceObjectId

        try {
            Write-Verbose "Initiating rotation of the bitlocker keys for endpoint [$($syncUri)]"
            $payload = @{"keepUserData" = if($KeepUserData) {$true} else { $false}}
            Invoke-RestMethod @aadRequestHeader -Method POST -uri $syncUri -Body ($payload | ConvertTo-Json -Compress) -ContentType 'application/json' -ErrorVariable ErrMsg
        }
        catch {
