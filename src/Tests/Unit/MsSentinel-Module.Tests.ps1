...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module MsSentinel.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain MsSentinel.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "MsSentinel.psm1"
        } #validPSM1
